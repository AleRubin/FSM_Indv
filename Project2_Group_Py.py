...
class FSM:
    def __init__(self): 
        ...
        self.reg_exp = {
            ...
            "clock_name_str": "clock.=.(.+)",
            "reset_name_str": "reset.=.(.+)",
            "states_str": "states.=.\((.+)\)",
            "inputs_str": "inputs.=.\((.+)\)",
            "inputs_name_str": "(\w+)\[(\d+)\]",
            "outputs_str": "outputs.=.\((.+)\)",
    ...
    def append_data(self):
        ...
        self.dictionary["stim_TB"] = self.get_string(self.fsm_file,3,self.reg_exp["transitions_str"])
    ...
    def get_input_data(self):# input_names = [[1,2,3,4,5,6][a,b,c,d,e]] input_names[0][2] :(
        for i in range(0, len(self.dictionary["stim"])):
            self.dictionary["stim_TB"][i]=self.dictionary["stim"][i].replace(","," & ")
            self.dictionary["stim_TB"][i]=self.dictionary["stim"][i].replace("=","=")
...
# fuction for write Tb of FSM

    def write_FSM_TB(self):
        fsm_file = open(str(self.dictionary["module_name"][0])+"_TB.sv",'w')
        #Name
        fsm_file.write("module "+self.dictionary["module_name"][0]+"_TB;\n")
        #Sensitive list
        fsm_file.write("    reg "+self.dictionary["clock_name"][0]+", "+self.dictionary["reset_name"][0]+"; \n")
        fsm_file.write("    reg ")
        for inp in range(0,len(self.dictionary["inputs"])):
            #write all inputs of buffer
            if(int(self.dictionary["inputs_length"][inp]) > 1):
                fsm_file.write("[" + str(int(self.dictionary["inputs_length"][inp])-1) +":0]")
            #write all inputs to one bit
            if  (len(self.dictionary["inputs"]) - inp)==1:
                fsm_file.write(self.dictionary["inputs"][inp]+";\n")
            else:
                fsm_file.write(self.dictionary["inputs"][inp]+",")
        # wite otputs w/buffer and single bit
        fsm_file.write("    wire ")
        for i in range(0,len(self.dictionary["outputs"])):
            #outputs with buffer
            if(int(fsm1.dictionary["outputs_length"][i]) > 1):
                fsm_file.write("[" + str(int(self.dictionary["outputs_length"][i])-1) + ":0]")                
            # outputs single bit
            if(len(self.dictionary["outputs"]) - i)==1:
                fsm_file.write(self.dictionary["outputs"][i]+";\n \n")
            else:
                fsm_file.write(self.dictionary["outputs"][i]+",")    
        # instantiate FSM design
        fsm_file.write("    "+self.dictionary["module_name"][0]+" UUT("+self.dictionary["clock_name"][0]+", "+self.dictionary["reset_name"][0]+",")
        for i in range(len(self.dictionary["inputs"])):
            fsm_file.write(self.dictionary["inputs"][i]+",")
        for i in range(len(self.dictionary["outputs"])):
            if (len(self.dictionary["outputs"])-i)==1:
                fsm_file.write(self.dictionary["outputs"][i]+");\n")
            else:
                fsm_file.write(self.dictionary["outputs"][i]+",")
        #Clock startup
        fsm_file.write("    always\n"+"    #1 \n"+"    "+self.dictionary["clock_name"][0]+" = ~"+self.dictionary["clock_name"][0]+";\n")
        
        #initialize dumpfile & dumpvars
        fsm_file.write("    initial\n"+"        begin \n")
        fsm_file.write("            $dumpfile(\""+self.dictionary["module_name"][0]+".vcd\");\n")
        fsm_file.write("            $dumpvars(0,"+self.dictionary["module_name"][0]+"_TB);\n \n")
        fsm_file.write("            "+self.dictionary["clock_name"][0]+"=1'b0;\n"+"            "+self.dictionary["reset_name"][0]+"=1'b1;\n")
        fsm_file.write("            #1\n"+"            "+self.dictionary["reset_name"][0]+"=1'b0;\n"+"            #1")
        
        #Initiates the possible trajectories of the WFTU
        self.get_input_data()
        index_states = []
        l = 1
        for j in range(0,len(self.dictionary["PS"])):
        ...
        S0pos=[p for p, v in enumerate(self.dictionary["NS"]) if v == "S0"]
        for k in range(len(index_states)+2):
            fsm_file.write("\n      "+self.dictionary["stim_TB"][k]+"; #1 \n")
        #suppressing transitions to the same state
        
        
        S0pos = [int(i) for i in S0pos]
        
        self.dictionary["PS"].pop(S0pos[0])
        self.dictionary["stim_TB"].pop(S0pos[0])
        self.dictionary["NS"].pop(S0pos[0])
        
        for l in range(0,len(self.dictionary["PS"])):
            fsm_file.write("\n      "+self.dictionary["stim_TB"][l]+";#1\n")
...
if __name__ == "__main__":
    fsm1 = FSM() 
    fsm1.fsm_file = "FSM_description.txt"
    fsm1.append_data()
    fsm1.write_FSM()
    fsm1.write_FSM_TB()